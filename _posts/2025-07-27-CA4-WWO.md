...
Much of my academic work takes place within relatively siloed structures, with limited time for sustained informal exchange between colleagues. As a result, communication and support around learning technologies are often mediated through shared digital systems and working environments rather than formal staff-development activity. These environments do two related things: they carry communication directly, through markup, comments, version history and annotations attached to the work itself, and they make communication possible in the first place, by giving collaborators access to the same code, data, model and outputs, so that discussion is grounded in a shared and inspectable object rather than in separate descriptions of what each person has done.

This is central to how DSG teams work. Participants share a secure virtual computing environment, including common datasets, code, outputs and GPU resources, so that methods can be examined, tested and developed collectively in real time. At a smaller scale, I use the same principle in supervision by placing markup and comments directly within a student's code (Evidence 2), so that the reasoning remains attached to the technical change and can be discussed against the same working object.

My strength is communicating through shared, inspectable artefacts that connect explanation, process and output. The limitation is that access to the same object does not guarantee shared understanding. A less experienced participant may still lack the framing needed to interpret what they are seeing or why it matters, in the same way that two people cannot fix a car together over the phone even if both are looking at an engine; they need to be under the bonnet together, looking at the same part, before a description means anything. I will therefore pair shared technical work more consistently with a short statement of purpose and expected outcome at the outset of a collaboration, so that a collaborator knows what they are looking at before they are asked to look at it.
